...
pub fn process(input: &str) -> String {
    ...
    let mut map: Map = vec![vec![false; 1000]; 1000];

    for instruction in instructions {
        instruction.apply(&mut map);
    }
    (0usize ..1000).cartesian_product(0usize ..1000).filter(|&(x, y)| map[y][x]).count().to_string()
}
...
mod tests {
    ...
    #[test]
    fn test_process() {
        let input = "turn on 0,0 through 99,99";
        assert_eq!("10000", process(input));
    }
}
