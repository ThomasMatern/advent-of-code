...
use nom::{IResult, Parser, character::complete, multi::separated_list1, sequence::separated_pair, bytes::complete::is_a};
...
fn count_arrangements<'a>(row: &'a [Spring], groups: &'a [usize], cache: &mut Cache<'a>) -> usize {
    ...
    if row[0] == Damaged || row[0] == Unknown {
        if row.len() >= groups[0] // enough room for group
            && !row[..groups[0]].contains(&Operational) { // space possibly valid for group
            
            if row.len() == groups[0] {  // last group consumes input
                count += count_arrangements(&row[groups[0]..], &groups[1..], cache)
            } 

            else if row[groups[0]] != Damaged { // terminated by not-damaged item
