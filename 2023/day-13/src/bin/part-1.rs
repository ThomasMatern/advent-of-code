...
fn process_pattern(pattern: &Pattern) -> usize {
    let mut cnt = process_pattern_horizontal(pattern).map(|x| x+1).unwrap_or(0) * 100;
    cnt += process_pattern_horizontal(&transpose_pattern(pattern)).map(|x| x+1).unwrap_or(0);
    return cnt;
}
