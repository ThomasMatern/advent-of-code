...
fn get_minimum_cubes(game: Game) -> (usize, usize, usize) {
    let (mut red, mut green, mut blue) = (0, 0, 0);
    for round in game.rounds {
        for set in round.sets {
            match set {
                CubeSet{colour:"red", count} if count > red => red = count,
                CubeSet{colour:"green", count} if count > green => green = count,
                CubeSet{colour:"blue", count} if count > blue => blue = count,
                _ => (),
            };
        };
    };
...
pub fn process(input: &str) -> String {
    parse_games(input)
        .into_iter()
        .map(|game| { get_minimum_cubes(game) } )
        .map(|(a, b, c)| { a * b * c })
        .sum::<usize>()
        .to_string()
}
