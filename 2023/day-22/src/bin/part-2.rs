...
pub fn process(input: &str) -> String {
    ...
    bricks.iter()
        .map(|brick| {
            ...
            let mut moving: HashSet<&Brick> = HashSet::new();
            queue.push_back(brick);

            while let Some(b) = queue.pop_front() {
                moving.insert(b);
                supports[&b].iter().for_each(|above| {
                    if is_supported[above].iter().all(|below| moving.contains(below)) {
                        queue.push_back(above);
                    }
                });
            }
            moving.len()-1 // don't count 'brick'
        })
        .sum::<usize>()
        .to_string()
}
