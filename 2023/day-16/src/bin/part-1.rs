...
use nom::{IResult, Parser, character::complete::{alphanumeric1, line_ending, one_of}, multi::{separated_list1, count, many1}, branch::alt, sequence::separated_pair};
...
pub fn process(i: &str) -> String {
    ...
    queue.insert(Beam{coord: (0,0), direction: &East});

    loop {
        if let Some(beam) = queue.pop_first() {
    ...
    energized.iter().map(|row| {
        row.iter().filter(|&value| *value).count()
    }).sum::<usize>().to_string()
