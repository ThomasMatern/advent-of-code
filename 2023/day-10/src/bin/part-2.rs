...
fn parse(i: &str) -> (Coord, Map, TileTypes) {
    ...
    else if sc.contains(&(start_pos.0, start_pos.1-1)) && sc.contains(&(start_pos.0, start_pos.1+1)) {
...
pub fn process(input: &str) -> String {
    ...
    let min_x = path.iter().min_by_key(|k| k.0).unwrap().0;
    let max_x = path.iter().max_by_key(|k| k.0).unwrap().0;
    let min_y = path.iter().min_by_key(|k| k.1).unwrap().1;
    let max_y = path.iter().max_by_key(|k| k.1).unwrap().1;

    (min_y ..= max_y)
        .map(|y| {
            let cnt = (min_x ..= max_x)
                .fold((0, false, Vertical),
                |(count, inside, last_cnr),x| {
