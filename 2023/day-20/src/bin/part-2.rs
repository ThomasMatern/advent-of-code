...
pub fn process(input: &str) -> String {
    ...
    // Get the input of rx
    let rx_inputs = state[&"rx"].inputs.iter().map(|(name, _)| *name).collect_vec();

    // Find the parents of that module
    let rx_parent_inputs = rx_inputs
        .iter()
        .flat_map(|input| {
            state[*input].inputs.iter().map(|(name, _)| *name)
        })
        .collect_vec();
            ...
            for &to_module_name in &map[&from_module_name].connections {
                ...
                if pulse == Low {
                    for parent_input in &rx_parent_inputs {
                        if !rx_parent_loops.contains_key(parent_input) {
                            if parent_input == &to_module_name {
    ...
    rx_parent_loops.iter()
        .fold(
            1isize,
            |acc, (_, factor)| lcm(acc, *factor)
        ).to_string()
}
